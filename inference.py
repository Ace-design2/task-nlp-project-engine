...
def analyze(text: str):
    ...
    for part in parts:
        ...
        if "deadline" in result and isinstance(result["deadline"], str):
            original_text = result["deadline"]
            
            # Pre-process common relative terms that dateparser might miss or parse poorly
            replacements = {
            ...
            text_to_parse = replacements.get(original_text.lower(), original_text)

            # Settings to prefer future dates and be strict about what is parsed
            settings = {
                'PREFER_DATES_FROM': 'future'
            }
            parsed_date = dateparser.parse(text_to_parse, settings=settings)
            
            result["deadline"] = {
                "text": original_text,
                "timestamp": parsed_date.strftime('%Y-%m-%dT%H:%M:%S') if parsed_date else None
            }
